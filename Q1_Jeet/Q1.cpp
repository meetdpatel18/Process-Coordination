...
void* performEditorTask(void* param){
    ...
    LOGGER::INFO("Task started for editor %d",editor);
    ...
    LOGGER::INFO("Task completed for editor %d", editor);
...
int main(){
    ...
    for(int i = 1;i <= N;i++){
        if(!res[i].empty()){
            ...
            for(int x : res[i])
                art += to_string(x) + " ";
            LOGGER::INFO("Editor %d accepted artiles : %s", i, art.c_str());
        }
